...
class IoU:
	...
	def _apply_modifier(self, volume1, volume2, operation: bool=0) -> float:
		...
		"""
		_volume = self._copy(volume1)
		deselect_all()
		select(_volume)
		print(_volume.name)
		bpy.ops.object.modifier_add(type='BOOLEAN')
		print(_volume.modifiers)
		_volume.modifiers['Boolean'].operation = self.operations[
			operation].upper()
		_volume.modifiers['Boolean'].object = volume2
		bpy.ops.object.modifier_apply(modifier='Boolean')
		result = self._get(_volume)
		select(_volume)
	...
	def _iou(self, v1, v2):
		_intersection = self._apply_modifier(v1, v2, 0)
